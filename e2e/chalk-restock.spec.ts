...
for(const width of [1280,390])test('restock owned chalk from low-stock email at '+width,async({page})=>{
 ...
 await page.goto('/');await page.getByRole('button',{name:/Continue Career/}).click();await expect(page.getByRole('heading',{name:'Upcoming & Recent Results',exact:true})).toBeVisible();await navigate('/inbox');await page.getByLabel('Inbox messages').getByRole('button',{name:/Chalk stock running low/}).click();await page.getByRole('button',{name:'Buy Chalk',exact:true}).click();
 await expect(page.getByLabel('Selected chalk stock')).toHaveText('1 unit');if(width<768) await page.getByRole('tab',{name:'Details',exact:true}).click();if(width<768) await page.getByRole('tab',{name:'Details',exact:true}).click();await page.getByRole('button',{name:/Buy another pack/}).click();await expect(page.getByLabel('Selected chalk stock')).toHaveText('6 units');
 ...
 await page.reload();await page.getByRole('button',{name:/Continue Career/}).click();await expect(page.getByRole('heading',{name:'Upcoming & Recent Results',exact:true})).toBeVisible();await navigate('/equipment/chalk-tips');await expect(page.getByLabel('Selected chalk stock')).toHaveText('6 units');expect((await readCareerSave(page)).player.cash).toBe(saved.player.cash);expect(errors).toEqual([]);
});
