...
test('training base shows costs and retains the selected membership after reload', async ({ page }) => {
  ...
  await expect(dialog).toContainText('£49,820');
...
test('hotel choices show nightly rates and a range, then charge the initial stay', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /Budget Hotel.*night/ }).first()).toBeVisible();
  await page.screenshot({ path: 'test-results/hotel-stay-desktop.png', fullPage: true });
  await page.setViewportSize({ width: 390, height: 844 });
  const hotel = page.getByRole('button', { name: /Guesthouse.*night/ });
  await expect(hotel).toBeVisible();
  await hotel.click();
  await expect(hotel).toHaveAttribute('aria-pressed', 'true');
  const expected = getTravelPackageEstimate(state, 'travel-1', 'hotel-5', event.id);
