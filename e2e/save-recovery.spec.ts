...
test('rotating snapshots, corrupt backup rejection and restore as a separate copy',async({page})=>{
 ...
 const oldSlot=await page.evaluate(key=>localStorage.getItem(key),ACTIVE_SAVE_SLOT_KEY);const original=await readCareerSave(page);
