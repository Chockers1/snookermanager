...
export function CoachAdvicePanel({ opponent = '', tournament, onUsePlan, compact = false }: { opponent?: string; tournament?: Tournament; compact?: boolean; onUsePlan?: (plan: 'Attack' | 'Balanced' | 'Safety') => void }) {
  ...
  return <details className="shrink-0 rounded-lg border border-border bg-surface text-xs">
    ...
    <div className="space-y-3 border-t border-border p-3 text-gray-300">
      {opponent && <div><p className="font-semibold text-green-400">Suggested approach: {advice.tactic}</p><p className="mt-1"><PlayerNames text={advice.tactical}/></p><p className="mt-1 text-[10px] text-gray-500"><PlayerNames text={advice.evidence}/></p>{onUsePlan && <button type="button" className="btn-secondary mt-2 text-xs" onClick={() => onUsePlan(advice.tactic)}>Use coach’s approach</button>}</div>}
      <p><PlayerNames text={advice.schedule}/></p>
...
export function MatchReviewPanel({ match }: { match: Match }) {
  ...
  return <section aria-label="Match review and development" className="card p-4">
    ...
    <div className="mt-3 grid gap-4 lg:grid-cols-2">
      <div><h3 className="text-xs font-semibold text-green-400">What the match showed</h3><ul className="mt-2 list-disc space-y-2 pl-4 text-xs text-gray-300">{review.evidence.map(line => <li key={line}><PlayerNames text={line}/></li>)}</ul><p className="mt-2 text-[10px] text-gray-500">Potting and safety rates are simulation estimates. They guide training; they do not prove why every frame was lost.</p></div>
      <div><h3 className="text-xs font-semibold text-green-400">{review.training.title}</h3><p className="mt-2 text-xs text-gray-300"><PlayerNames text={review.training.reason}/></p><p className="mt-2 text-xs text-gray-400">{review.training.sessions} Keep event and travel days protected.</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={project?.status === 'active'} className="btn-primary text-xs" onClick={() => actOnCareer({ type: 'project', kind: review.training.kind })}>{project?.status === 'active' ? 'Development project already active' : 'Start recommended project'}</button><Link to="/training" className="btn-secondary text-xs">Choose training sessions</Link></div></div>
    </div>
