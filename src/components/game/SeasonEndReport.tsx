...
export function SeasonEndReport({ report }: { report: Report }) {
  ...
  return <section aria-label="End of season report" className="mt-2 space-y-2 text-[11px] leading-4">
    ...
    {report.emergingStars && <details className="rounded-lg border border-border px-2 py-1.5"><summary className="cursor-pointer font-semibold text-white">Emerging stars to watch{report.emergingStars[0] ? ' · ' + report.emergingStars[0].name + ' · OVR ' + report.emergingStars[0].overall + ' / POT ' + report.emergingStars[0].potential : ' · No standout recorded'}</summary>
      ...
      <ul className="mt-1 divide-y divide-border/50">{report.emergingStars.map(star => <li key={star.id} className="py-2"><p><PlayerLink name={star.name} id={star.id}/> · {star.nation} · Age {star.age}</p><p className="text-[10px] text-gray-400">{star.detail}</p></li>)}</ul>
    </details>}
    <div className="grid gap-2 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]">
      ...
      <div className="min-w-0 rounded border border-border px-2 py-1.5"><h3 className="mb-1 font-semibold text-white">Closing world rankings</h3>{report.finalRankings?.length ? <ol className="space-y-1">{report.finalRankings.map(row => <li key={row.ranking} className="flex flex-wrap justify-between gap-x-2"><span className="text-gray-200">#{row.ranking} <PlayerLink name={row.playerName}/></span><span className="text-[10px] text-gray-400">{row.points.toLocaleString('en-GB')} pts</span></li>)}</ol> : <p className="text-gray-400">{report.worldNumberOne ? '#1 ' + report.worldNumberOne : 'Closing standings were not retained.'}</p>}<p className="mt-2 text-[10px] text-gray-400">Season totals use surviving event records. Cash change, when available, covers the dates shown.</p></div>
    </div>
  </section>;
}
