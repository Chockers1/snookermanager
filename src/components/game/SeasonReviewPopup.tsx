...
export function SeasonRankings({ review, playerName, onNavigate }: { review: Review; playerName: string; onNavigate?:()=>void }) {
  ...
  return <section aria-label="Final world rankings" className="min-w-0 rounded-lg border border-border p-3">
    ...
    {rows.length ? <ol className="space-y-1 text-xs">{rows.map(row => <li key={row.playerName} className={'flex gap-2 rounded px-2 py-1 ' + (row.playerName === playerName ? 'bg-green-500/10 text-green-400' : 'text-gray-300')}><span className="w-6 shrink-0">#{row.ranking}</span><span className="min-w-0 flex-1 break-words"><PlayerLink name={row.playerName} onNavigate={onNavigate}/></span></li>)}</ol> : <p className="text-xs text-gray-400">Closing standings were not saved for this older review.</p>}
  </section>;
}
