...
export function SeasonTourChangesReport({report}:{report:SeasonTourChanges}) {
  return <section aria-label="Season tour changes" className="mt-3 min-w-0 max-w-3xl space-y-3">
    ...
    <div className="rounded-lg border border-border bg-background/30"><div className="border-b border-border px-4 py-2"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-gray-500">Report summary</p></div><div className="divide-y divide-border/70">{report.sections.map(section=><section key={section.id} aria-label={section.title} className="min-w-0 px-4 py-3">
      <div className="flex items-center justify-between gap-2"><h3 className="text-xs font-semibold text-gray-300">{section.title}</h3><span className="rounded bg-surface-light px-1.5 py-0.5 text-[10px] text-white">{section.people.length}{!report.complete && section.id !== 'emergingStars'?' recorded':''}</span></div>
      {section.id === 'emergingStars' && <p className="mt-1 text-[10px] leading-4 text-gray-500">Ratings as of {section.asOf ?? report.asOf} · Up to five prospects aged 25 or younger. Youth: OVR 70+ or POT 90+; ages 22–25: OVR 78+ or POT 92+. Potential is not guaranteed.</p>}
      {section.people.length ? <><ul className="mt-1 divide-y divide-border/50">{section.people.slice(0,2).map(person=><PersonRow key={person.id} person={person}/>)}</ul>{section.people.length>2 && <details className="mt-1"><summary className="cursor-pointer text-[10px] text-green-400">Show {section.people.length-2} more</summary><ul className="mt-1 divide-y divide-border/50">{section.people.slice(2).map(person=><PersonRow key={person.id} person={person}/>)}</ul></details>}</>:<p className="py-2 text-[10px] leading-4 text-gray-500">{section.id === 'emergingStars' ? 'No prospects with recorded ratings meet the spotlight criteria.' : report.complete?'No changes this season.':'No confirmed changes in the retained records.'}</p>}
    </section>)}</div></div>
