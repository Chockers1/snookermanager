...
export function GroupFixtures({ rounds, playerName, currentRound }: { rounds: BracketRound[]; playerName: string; currentRound?: string | null }) {
  ...
  const round = rounds.find(r=>r.label===stage) ?? rounds[0];
  ...
  return <section aria-label="Group standings and fixtures" className="h-full min-h-0 space-y-3 overflow-y-auto p-1 text-xs">
    <div className="flex flex-wrap items-center gap-2">
      <label>Stage <select aria-label="Group stage" value={round?.label} onChange={e=>{setStage(e.target.value);setChosenGroup('');}} className="ml-2 min-h-10 rounded border border-border bg-surface px-2">{rounds.map(r=><option key={r.label}>{r.label}</option>)}</select></label>
    ...
    {group ? <>
      ...
      <div className="divide-y divide-border">{group.matches.map((m,i)=><div key={m.id} className={`flex items-center justify-between gap-3 px-2 py-2 ${m.id===upcoming?.id?'rounded bg-amber-500/10 text-amber-200':''}`}><span className="text-gray-500">{i+1}</span><span className="flex-1">{m.top.name}</span><b className="shrink-0">{fixtureComplete(m)?`${m.top.score}–${m.bottom.score}`:'vs'}</b><span className="flex-1 text-right">{m.bottom.name}</span><span className="w-16 text-right text-[10px]">{fixtureComplete(m)?'Played':m.id===upcoming?.id?'Next match':'Scheduled'}</span></div>)}</div>
    </> : round?.matches.length ? <div className="rounded border border-border p-4">{round.matches.map(m=><p key={m.id} className="flex justify-between gap-3"><span>{m.top.name}</span><b>{fixtureComplete(m)?`${m.top.score}–${m.bottom.score}`:'vs'}</b><span>{m.bottom.name}</span></p>)}<p className="mt-3 text-gray-400">{round.label} · best of {round.bestOf ?? 5}</p></div> : <p className="p-3 text-gray-400">Groups are drawn when the previous stage finishes.</p>}
  </section>;
}
