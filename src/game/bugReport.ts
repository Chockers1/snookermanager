import { materializeCareerHistory } from './careerArchive';
...
export async function downloadBugReport(state:GameState|null,description:string,route:string){const report=buildBugReport(state,description,route);if(report.save&&(report.save as GameState).historyArchive) report.save=await materializeCareerHistory(report.save as GameState);const url=URL.createObjectURL(new Blob([JSON.stringify(report,null,2)],{type:'application/json'}));const link=document.createElement('a');link.href=url;link.download='snooker-bug-report-'+new Date().toISOString().slice(0,10)+'.json';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
