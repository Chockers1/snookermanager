import { getMissingTournamentEquipment, getNextEligibleTournament, type GameState } from '../hooks/useGameState';
...
export function reconcileFirstWeekGuide(state:GameState):GameState {
 ...
 if(guide.equipmentReviewed||getMissingTournamentEquipment(state.equipment).length===0)done.add('equipment');
...
/** A guided entry opens a real eligible event without booking it or advancing time. */
export function firstWeekEntryRoute(state: GameState): string {
 const event = getNextEligibleTournament(state);
 return event ? `/calendar?guide=entry&tournament=${encodeURIComponent(event.id)}` : '/calendar?guide=entry';
}
