...
describe('public player history and rewards',()=>{
 ...
 it('keeps season totals after bracket compaction and marks already-missing older details honestly',()=>{
  ...
  Object.values(compact.rollingRankings!.events)[0].bracket=[];
 ...
 it('retains final placing and exhibition identity when old CPU draws wait for durable archival',()=>{const {state,opponent}=playerWorldFixture();state.currentDate='2030-01-01';const compact=compactRankingLedger(state);const result=playerEventHistory(compact,opponent.playerName)[0];expect(result.result).toBe('Winner');expect(result.category).toBe('Exhibition achievement');expect(result.matches).toHaveLength(1);expect(compactRankingLedger(compact).rollingRankings?.events).toEqual(compact.rollingRankings?.events)});
