...
export function finishTeamVisit(before:LiveMatchState,after:LiveMatchState):LiveMatchState {
 ...
 // Foul penalties are already credited to the opposing frame score; they are not negative scoring.
 const members=base.members.map((p,i)=>i===index?{...p,visits:p.visits+Number(before.currentBreak===0),pots:p.pots+Number(log.success&&log.points>0),fouls:p.fouls+Number(log.foulOccurred),points:p.points+Math.max(0,log.points),highestBreak:Math.max(p.highestBreak,log.breakTotal),confidence:frameEnded?p.confidence:index<2?after.playerConfidence:after.opponentConfidence,fatigue:frameEnded?p.fatigue:index<2?after.playerFatigue:after.opponentFatigue}:p);
