...
export function calculateSponsorMatchBonus(
  ...
  const rankingTitle = clause.match(/ranking title\s*\+£([\d,]+)/i);
  if (rankingTitle && match.result === 'Won' && round === 'final' && ['World Ranking','One-Year'].includes(tournament.rankingType ?? '')) [amount, reason] = [Number(rankingTitle[1].replaceAll(',', '')) + (/event win/i.test(clause) ? amounts[0] : 0), 'ranking title'];
  else if (/event win/i.test(clause) && match.result === 'Won' && round === 'final') [amount, reason] = [amounts[0], 'event win']
  ...
  return { amount, reason, key: persistentMilestone ? `career:${reason}` : sponsor.signedSeason ? `${tournament.id}:${tournament.startDate}:${reason}` : `${tournament.id}:${reason}` }
}
