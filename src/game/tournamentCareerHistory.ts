...
export function recoverTournamentArchive(state: GameState): GameState {
  ...
  for(const ledger of Object.values(state.rollingRankings?.events ?? {})) {
    ...
    recovered.push({id:ledger.season+'-'+ledger.tournamentId,season:ledger.season,tournamentId:ledger.tournamentId,tournamentName:ledger.name,eventType:event.eventClass??event.type,tourCircuit:event.tourCircuit??event.type,stageId:event.stageId??null,formatId:event.formatId,location:event.location,startDate,endDate:ledger.completedOn,status:'Completed',result:finish,rounds:rounds.map(r=>r.round+': '+r.result+' '+r.playerFrames+'-'+r.opponentFrames),roundResults:rounds,matchesPlayed:rounds.length,wins:rounds.filter(r=>r.result==='Won').length,losses:rounds.filter(r=>r.result==='Lost').length,prizeMoney:ledger.prizeAwards?.[state.player.fullName]??earning?.amount??0,rankingPoints:earning?.amount??0,highestBreak:0,centuries:0,fatigueChange:0,entryFee:0,bookedTravelCost:0,recoveredFromLedger:{prizeKnown:ledger.prizeAwards?.[state.player.fullName]!==undefined||Boolean(earning)}});
  }
