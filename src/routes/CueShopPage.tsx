...
function getActiveTab(pathname: string, hash: string): EquipmentTab {
  if (pathname === "/equipment/chalk-tips")
    return hash === "#tips" ? "tips" : "chalk";
...
function StatLine({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex items-center gap-2">
      <span className="w-24 text-[10px] capitalize text-gray-400">{label}</span>
      <div className="flex-1">
        <ProgressBar value={value} compact />
      </div>
      <span className="w-7 text-right text-[10px] text-white">{value}</span>
    </div>
  );
}
...
export function CueShopPage() {
  ...
  const [selectedCueId, setSelectedCueId] = useState(
    gameState.equipment.currentCueId ?? cueMarketplaceCatalog[0]?.id ?? "",
  );
  const [selectedChalkId, setSelectedChalkId] = useState(
    gameState.equipment.currentChalkId ?? chalkCatalog[0]?.id ?? "",
  );
  const [selectedTipId, setSelectedTipId] = useState(
    gameState.equipment.currentTipId ?? tipCatalog[0]?.id ?? "",
  );
  const [selectedCaseId, setSelectedCaseId] = useState(
    gameState.equipment.currentCaseId ?? caseCatalog[0]?.id ?? "",
  );
  const [selectedTableId, setSelectedTableId] = useState(
    gameState.equipment.currentTableId ?? tableSetupCatalog[0]?.id ?? "",
  );
  const [selectedActionId, setSelectedActionId] = useState(
    maintenanceActionCatalog[0]?.id ?? "",
  );
  const [sortMode, setSortMode] = useState<"price" | "performance">(
    "performance",
  );
  ...
  const currentCue = gameState.equipment.currentCueId
    ...
    : null;
  const currentChalk = gameState.equipment.currentChalkId
    ? (chalkCatalog.find(
        (chalk) => chalk.id === gameState.equipment.currentChalkId,
      ) ?? null)
    : null;
  const currentTip = gameState.equipment.currentTipId
    ? (tipCatalog.find((tip) => tip.id === gameState.equipment.currentTipId) ??
      null)
    : null;
  const currentCase = gameState.equipment.currentCaseId
    ? (caseCatalog.find(
        (entry) => entry.id === gameState.equipment.currentCaseId,
      ) ?? null)
    : null;
  const currentTable = gameState.equipment.currentTableId
    ? (tableSetupCatalog.find(
        (entry) => entry.id === gameState.equipment.currentTableId,
      ) ?? null)
    : null;
  const currentCueState = gameState.equipment.currentCueId
    ? gameState.equipment.cueStates[gameState.equipment.currentCueId]
    : null;

  const visibleCues = cueMarketplaceCatalog
    .filter((cue) =>
      ownedOnly ? gameState.equipment.cuesOwned.includes(cue.id) : true,
    )
    .slice()
    .sort((left, right) =>
      sortMode === "price"
        ...
        : right.touch +
          ...
          right.stability +
          right.durability -
          (left.touch + left.spinControl + left.stability + left.durability),
    );
  const visibleChalk = chalkCatalog
    .filter((chalk) =>
      ownedOnly ? gameState.equipment.chalkOwned.includes(chalk.id) : true,
    )
    .slice()
    .sort((left, right) =>
      sortMode === "price"
  ...
  const visibleTips = tipCatalog
    .filter((tip) =>
      ownedOnly ? gameState.equipment.tipsOwned.includes(tip.id) : true,
    )
    .slice()
    .sort((left, right) =>
      sortMode === "price"
        ...
        : right.durability +
          ...
          right.feel +
          right.consistency -
          (left.durability + left.spinControl + left.feel + left.consistency),
    );
  const visibleCases = caseCatalog
    .filter((entry) =>
      ownedOnly ? gameState.equipment.casesOwned.includes(entry.id) : true,
    )
    .slice()
    .sort((left, right) =>
      sortMode === "price"
  ...
  const visibleTables = tableSetupCatalog
    .filter((entry) =>
      ownedOnly ? gameState.equipment.tablesOwned.includes(entry.id) : true,
    )
    .slice()
    .sort((left, right) =>
      sortMode === "price"
        ? left.monthlyRental - right.monthlyRental
        : right.clothSpeed +
          right.cushionResponse +
          right.pocketForgiveness +
          right.napQuality -
          (left.clothSpeed +
            left.cushionResponse +
            left.pocketForgiveness +
            left.napQuality),
    );

  const selectedCue =
    cueMarketplaceCatalog.find((cue) => cue.id === selectedCueId) ??
    visibleCues[0] ??
    cueMarketplaceCatalog[0];
  const selectedChalk =
    chalkCatalog.find((chalk) => chalk.id === selectedChalkId) ??
    visibleChalk[0] ??
    chalkCatalog[0];
  const selectedTip =
    tipCatalog.find((tip) => tip.id === selectedTipId) ??
    visibleTips[0] ??
    tipCatalog[0];
  const selectedCase =
    caseCatalog.find((entry) => entry.id === selectedCaseId) ??
    visibleCases[0] ??
    caseCatalog[0];
  const selectedTable =
    tableSetupCatalog.find((entry) => entry.id === selectedTableId) ??
    visibleTables[0] ??
    tableSetupCatalog[0];
  const selectedAction =
    maintenanceActionCatalog.find((action) => action.id === selectedActionId) ??
  ...
  const setupBonus = Math.round(
    (selectedChalk.grip +
      selectedChalk.cleanContact +
      selectedTip.spinControl +
      selectedTip.feel +
      (currentCue?.touch ?? 50)) /
      5 /
      10,
  ...
  function renderCards() {
        ...
        return (
          ...
          >
            ...
            <div className="space-y-1.5">
              ...
              <StatLine label="Contact" value={chalk.cleanContact} />
              <StatLine label="Spin" value={chalk.spinTransfer} />
              <StatLine label="Consistency" value={chalk.consistency} />
            </div>
      ...
      return visibleTips.map((tip) => {
        ...
        return (
          ...
          >
            <div className="mb-3 flex items-start justify-between">
              <div>
                <h3 className="text-sm font-semibold text-white">{tip.name}</h3>
                <p className="mt-0.5 text-lg font-bold text-green-400">
            ...
            <div className="space-y-1.5">
              ...
              <StatLine label="Spin" value={tip.spinControl} />
              <StatLine label="Feel" value={tip.feel} />
              <StatLine label="Consistency" value={tip.consistency} />
            </div>
            <div className="mt-3 flex items-center justify-between border-t border-border pt-2 text-[10px]">
        ...
        return (
          ...
          >
            ...
            <div className="space-y-1.5">
              ...
              <StatLine label="Storage" value={entry.storage} />
              <StatLine label="Travel" value={entry.travelComfort} />
              <StatLine label="Presentation" value={entry.presentation} />
            </div>
            <div className="mt-3 flex items-center justify-between border-t border-border pt-2 text-[10px]">
        ...
        return (
          ...
          >
            ...
            <div className="space-y-1.5">
              ...
              <StatLine label="Cushions" value={table.cushionResponse} />
              <StatLine label="Pockets" value={table.pocketForgiveness} />
              <StatLine label="Nap" value={table.napQuality} />
            </div>
            <div className="mt-3 flex items-center justify-between border-t border-border pt-2 text-[10px]">
    ...
    if (activeTab === "maintenance")
      return maintenanceActionCatalog.map((action) => (
        <button
          key={action.id}
          type="button"
        ...
        >
          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            {action.restoration.map((item) => (
              <div
                key={item.label}
                className="rounded bg-surface-light/50 p-2 text-center"
              >
                <p className="text-[10px] text-gray-500">{item.label}</p>
                <p className="font-bold text-green-400">+{item.value}</p>
              </div>
            ))}
          </div>
      ...
      return (
        ...
        >
            ...
            <div>
              ...
              <div className="mt-0.5 flex items-center gap-2">
                <span className="text-lg font-bold text-green-400">
                  {formatMoney(cue.price)}
                </span>
                <span className="rounded bg-surface-light px-1.5 py-0.5 text-[10px] text-gray-400">
                  {cue.tier}
                </span>
              </div>
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <StatLine label="Spin" value={cue.spinControl} />
            <StatLine label="Stability" value={cue.stability} />
        ...
        <>
          ...
          <div className="mt-3 space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-gray-500">Price</span>
              <span className="font-bold text-green-400">
                {formatMoney(selectedTip.cost)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Hardness</span>
              <span className="text-white">{selectedTip.hardness}</span>
            </div>
          </div>
          <button
            type="button"
            className="btn-primary mt-4 w-full justify-center text-xs"
            onClick={() => buyTip(selectedTip.id)}
          >
            <ShoppingCart className="h-3.5 w-3.5" />{" "}
            {gameState.equipment.tipsOwned.includes(selectedTip.id)
              ? "Equip Tip"
              : "Buy Tip"}
          </button>
        </>
        ...
        <>
          ...
          <div className="mt-3 space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-gray-500">Price</span>
              <span className="font-bold text-green-400">
                {formatMoney(selectedCase.price)}
              </span>
            </div>
            <div className="flex justify-between">
          ...
          <button
            type="button"
            className="btn-primary mt-4 w-full justify-center text-xs"
            onClick={() => buyCase(selectedCase.id)}
          >
            <ShoppingCart className="h-3.5 w-3.5" />{" "}
            {gameState.equipment.casesOwned.includes(selectedCase.id)
              ? "Equip Case"
              : "Buy Case"}
          </button>
        </>
        ...
        <>
          ...
          <div className="mt-3 space-y-2 text-xs">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-green-400">
                {formatMoney(selectedTable.monthlyRental)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Tier</span>
          ...
          <button
            type="button"
            className="btn-primary mt-4 w-full justify-center text-xs"
            onClick={() => buyTableSetup(selectedTable.id)}
          >
            <ShoppingCart className="h-3.5 w-3.5" />{" "}
            {gameState.equipment.tablesOwned.includes(selectedTable.id)
              ? "Activate Membership"
              : "Join Membership"}
          </button>
        </>
        ...
        <>
          ...
          <div className="mt-3 space-y-2 text-xs">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-green-400">
                {formatMoney(selectedAction.cost)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Time</span>
              <span className="text-white">{selectedAction.timeRequired}</span>
            </div>
          </div>
          <button
            type="button"
          ...
          >
            <ShieldCheck className="h-3.5 w-3.5" /> Perform Maintenance
          </button>
        </>
      );
    return (
      <>
        ...
        <button
          type="button"
          className="btn-primary mt-4 w-full justify-center text-xs"
          onClick={() => buyCue(selectedCue.id)}
        >
          <ShoppingCart className="h-3.5 w-3.5" />{" "}
          {gameState.equipment.cuesOwned.includes(selectedCue.id)
            ? "Equip Cue"
            : `Buy Cue - ${formatMoney(selectedCue.price)}`}
        </button>
      </>
    );
  }
      ...
      <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-none">
          <button
            ...
            onClick={() =>
              setSortMode((mode) =>
                mode === "price" ? "performance" : "price",
              )
            }
          >
            <SlidersHorizontal className="h-3.5 w-3.5" />{" "}
            {sortMode === "price" ? "Sort: Price" : "Sort: Performance"}
          </button>
          <button
            type="button"
            className="btn-secondary min-h-10 justify-center whitespace-nowrap text-xs"
          ...
          >
            {ownedOnly ? "Owned Only" : "All Items"}
          </button>
        </div>
      </div>
      <div
