...
export function LiveMatchPage() {
  const { gameState, simulateLiveVisit, simulateLiveFrame, simulateLiveMatch, applyLiveCoachCue, concedeLiveFrame, updateLiveMatchTactics } = useGame()
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => simulateLiveVisit(), 700)
    return () => window.clearTimeout(timer)
  }, [autoPlaying, concedeConfirmationOpen, liveMatch, simulateLiveVisit])
      ...
      <main className="mx-auto mt-2 grid min-h-0 w-full max-w-[1580px] flex-1 gap-2 xl:grid-cols-[350px_minmax(0,1fr)_330px]">
        ...
        <section className="grid min-h-0 gap-2 xl:grid-rows-[auto_minmax(0,1fr)]">
          ...
          <div className="grid min-h-0 gap-2 lg:grid-cols-2">
            ...
            <div className="card flex min-h-64 flex-col overflow-hidden"><div className="card-header"><div><h2 className="text-sm font-bold">Frame Log</h2><p className="text-[9px] text-gray-500">Oldest to newest</p></div><span className="text-[10px] text-green-400">Live · follows play</span></div><div ref={frameLogRef} data-testid="frame-log" aria-live="polite" className="scrollbar-thin min-h-0 flex-1 overflow-auto px-3 py-2 text-[11px]">{chronologicalFrameLog.map((feed, index) => { const isLatest = index === chronologicalFrameLog.length - 1; return <div key={feed.id} className={`relative grid grid-cols-[38px_minmax(0,1fr)] gap-2 border-l pb-2 pl-3 ${isLatest ? 'border-green-400' : 'border-border'}`}><span className={`absolute -left-1 top-2 h-2 w-2 rounded-full ${feed.tone === 'green' ? 'bg-green-400' : feed.tone === 'red' ? 'bg-red-400' : feed.tone === 'amber' ? 'bg-amber-400' : 'bg-blue-400'}`} /><span className="pt-1.5 text-gray-500">{feed.time}</span><div className={`rounded px-2 py-1.5 ${isLatest ? 'bg-green-500/10 ring-1 ring-green-500/20' : 'bg-surface-light/35'}`}><span className="mb-0.5 block text-[8px] font-bold uppercase tracking-wider text-gray-500">{feed.actor}</span><span className={feed.tone === 'green' ? 'text-green-300' : feed.tone === 'red' ? 'text-red-300' : feed.tone === 'amber' ? 'text-amber-300' : 'text-gray-300'}>{feed.text}</span></div></div>})}</div><div data-testid="frame-log-current-break" className="shrink-0 border-t border-border bg-black/20 px-3 py-2"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-[8px] font-bold uppercase tracking-[0.16em] text-gray-500">Now at the table</p><p className={`truncate text-xs font-semibold ${atTableIsPlayer ? 'text-green-300' : 'text-amber-300'}`}>{liveMatch.playerAtTable}</p></div><div className="text-right"><p className="text-[8px] uppercase text-gray-500">Current break</p><p className="text-2xl font-black text-green-400">{liveMatch.currentBreak}</p></div></div></div></div>
          </div>
        </section>
