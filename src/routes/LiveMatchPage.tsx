...
import { TeamLivePanel } from '../components/career/SeasonLifePanels';
...
function PlayerScore({ name, points, role, atTable, side, condition }: { name: string; points: number; role: string; atTable: boolean; side: 'player' | 'opponent'; condition?: { confidence: number; fatigue: number; pressure: number } }) {
  ...
  return (
    <div className={`flex min-w-0 items-center gap-3 px-4 py-2 sm:px-6 ${playerSide ? 'border-green-500/30 bg-green-500/10' : 'border-red-500/30 bg-red-500/5'} ${playerSide ? 'lg:border-r' : 'lg:flex-row-reverse lg:border-l lg:text-right'}`}>
      ...
      <div className="min-w-0 flex-1"><p className={`text-[9px] font-semibold uppercase tracking-wider ${atTable ? 'text-green-400' : 'text-gray-500'}`}>{atTable ? 'At table' : 'Waiting'}</p><h2 className="truncate text-base font-bold sm:text-xl"><PlayerNames text={name}/></h2><p className="truncate text-[10px] text-gray-400 sm:text-xs">{role}</p>
...
export function LiveMatchPage() {
  ...
  const teamWasLive = useRef(false)
  useEffect(() => { if (liveMatch?.teamContext) teamWasLive.current = true; if (!liveMatch && teamWasLive.current) navigate('/career/teams', { replace: true }); }, [liveMatch, navigate])
  const tournament = liveMatch ? gameState.tournaments.find((item) => item.id === liveMatch.tournamentId) ?? (liveMatch.teamContext ? {name:gameState.careerDepth?.seasonLife?.teams.find(e=>e.id===liveMatch.teamContext!.eventId)?.name??'Team event',location:'Host club'} : null) : null

  useEffect(() => {
    if (liveMatch?.status === 'Completed') {
      navigate(liveMatch?.teamContext ? '/career/teams' : '/match/result', { replace: true })
    }
  }, [liveMatch?.status, liveMatch?.teamContext, navigate])
  ...
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (event.target instanceof HTMLInputElement || event.target instanceof HTMLSelectElement || event.target instanceof HTMLTextAreaElement || event.target instanceof HTMLButtonElement || event.target instanceof HTMLAnchorElement) return
      if (event.key === ' ') {
  ...
  const team = liveMatch.teamContext
  const atTableDisplay = team ? team.members[team.order[team.turn]].name : liveMatch.playerAtTable
  const sideCondition = (home: boolean) => {
    const members = team?.members.filter((_, index) => team.order.includes(index) && (index < 2) === home)
    return {confidence: members?.length ? members.reduce((sum,p)=>sum+p.confidence,0)/members.length : home ? liveMatch.playerConfidence : liveMatch.opponentConfidence, fatigue: members?.length ? members.reduce((sum,p)=>sum+p.fatigue,0)/members.length : home ? liveMatch.playerFatigue : liveMatch.opponentFatigue, pressure: liveMatch.pressureValue}
  }
  ...
  return (
    <div className="min-h-screen overflow-auto bg-[#050b12] p-2.5 text-white sm:p-3 xl:flex xl:h-screen xl:min-h-0 xl:flex-col xl:overflow-hidden" data-testid="live-match-score-centre">
      <TeamLivePanel />
      {liveMatch.objectives?.length ? <div aria-label="Live personal objectives" className="mb-2 flex shrink-0 flex-wrap gap-x-4 gap-y-1 rounded border border-green-500/20 bg-green-500/5 px-3 py-1 text-[11px] text-green-300"><strong>Personal objectives</strong>{liveMatch.objectives.map(goal => <span key={goal.id}>{goal.label}: {goal.id === "frames" ? liveMatch.playerFrames : liveMatch.playerHighestBreak}/{goal.target}</span>)}</div> : null}
      {sessionBreak && <CareerEditor required title={sessionBreak.kind === 'overnight' ? 'Overnight session break' : sessionBreak.kind === 'session' ? 'Session break' : 'Mid-session interval'} onClose={() => setAutoPlaying(false)}>
        <div className="space-y-4 overflow-y-auto border-t border-border p-4 text-sm">
          <p className="font-semibold"><PlayerLink name={liveMatch.playerName}/> {liveMatch.playerFrames}–{liveMatch.opponentFrames} <PlayerLink name={liveMatch.opponentName}/> · {sessionBreak.afterFrame} frames played</p>
          <p className="text-gray-400"><PlayerNames text={liveMatch.coachPrompt.note}/></p><ul className="list-inside list-disc text-gray-300">{sessionAssessment(liveMatch).map(line => <li key={line}><PlayerNames text={line}/></li>)}</ul><p>Fatigue {Math.round(liveMatch.playerFatigue)}% · confidence {Math.round(liveMatch.playerConfidence)}% · highest break {liveMatch.playerHighestBreak}</p>
      ...
      <header className="mx-auto flex w-full max-w-[1580px] shrink-0 items-center gap-3 rounded-xl border border-border bg-surface/85 px-3 py-2">
        <button type="button" aria-label={team ? "Return to team event" : "Return to Tournament Hub"} onClick={() => navigate(team ? '/career/teams' : '/tournaments/hub')} className="grid h-11 w-11 shrink-0 place-items-center rounded-lg border border-border bg-surface-light text-gray-300 hover:border-green-500/50 hover:text-white"><Menu className="h-5 w-5" /></button>
        <div className="min-w-0 flex-1"><h1 className="truncate text-sm font-bold sm:text-base">{tournament?.name ?? 'Live Match'} · {liveMatch.round}</h1><p className="mt-0.5 flex items-center gap-1.5 truncate text-[10px] text-gray-400"><CalendarDays className="h-3 w-3" /> {liveMatch.bestOf === 4 ? 'Up to four frames · draws allowed' : `Best of ${liveMatch.bestOf}`} · {tournament?.location ?? liveMatch.table} · {liveMatch.special?.rules.includes('shootOut') ? `${Math.max(0, Math.ceil(600 - liveMatch.special.elapsedSeconds))}s remaining · ${liveMatch.shotClock}s shot clock` : formatClock(liveMatch.timeElapsedMinutes)}</p></div>
        <div className="hidden items-center gap-5 text-right sm:flex"><div><p className="text-[9px] uppercase text-gray-500">Frame</p><p className="font-bold">{liveMatch.currentFrame} of {liveMatch.bestOf}</p></div><div><p className="text-[9px] uppercase text-gray-500">At table</p><p className="max-w-32 truncate text-sm font-semibold text-green-300"><PlayerLink name={atTableDisplay}>{getShortName(atTableDisplay)}</PlayerLink></p></div></div>
      </header>
      ...
      <section className="mx-auto mt-2 grid min-h-28 w-full max-w-[1580px] shrink-0 overflow-hidden rounded-xl border border-border bg-surface/90 lg:h-32 lg:grid-cols-[1fr_250px_1fr]">
        <PlayerScore name={liveMatch.playerName} points={liveMatch.playerPoints} role={team ? team.kind === 'doubles' ? 'Pair condition · average' : 'Team singles' : gameState.player.competitiveStatus ?? gameState.player.careerStage} atTable={atTableIsPlayer} side="player" condition={sideCondition(true)} />
        ...
        <PlayerScore name={liveMatch.opponentName} points={liveMatch.opponentPoints} role={team ? team.kind === 'doubles' ? 'Pair condition · average' : 'Team singles' : `${liveMatch.opponentArchetype} · #${liveMatch.opponentRanking}`} atTable={!atTableIsPlayer} side="opponent" condition={sideCondition(false)} />
      </section>
      ...
      <main className="mx-auto mt-2 grid min-h-0 w-full max-w-[1580px] flex-1 gap-2 xl:grid-cols-[350px_minmax(0,1fr)_330px]">
        <section className="card flex min-h-[31rem] flex-col overflow-hidden xl:min-h-0" data-testid="match-playback-controls">
          <div className="border-b border-border px-4 py-3"><p className={`text-[9px] font-semibold uppercase tracking-[0.2em] ${autoPlaying ? 'text-green-400' : 'text-gray-500'}`}>Match Control</p><div className="mt-1 flex items-start justify-between gap-3"><div><h2 className="text-xl font-bold">Frame {liveMatch.currentFrame} in progress</h2><p className="mt-1 text-xs text-gray-400">{getFramePhase(liveMatch)} · {getRemainingTablePoints(liveMatch)} points remain</p></div><div className="rounded-lg border border-border bg-black/20 px-3 py-1.5 text-center"><p className="text-[8px] uppercase text-gray-500">Break</p><p className="text-xl font-black text-green-400">{liveMatch.currentBreak}</p></div></div></div>
          <div tabIndex={0} aria-label="Match commentary and current score" className="space-y-3 p-4 text-xs"><p className="text-xs leading-relaxed text-gray-300">Choose the pace. Scores, frame events and statistics update automatically.</p><div className="flex items-center justify-between border-b border-border pb-3"><span className="text-gray-500">At table</span><b className={atTableIsPlayer ? 'text-green-300' : 'text-amber-300'}><PlayerNames text={atTableDisplay}/></b></div><div className="flex items-center justify-between border-b border-border pb-3"><span className="text-gray-500">Current score</span><b>{liveMatch.playerPoints}–{liveMatch.opponentPoints}</b></div><div className="rounded-lg bg-surface-light/45 p-3 text-[11px] leading-relaxed text-gray-400"><p className="font-semibold text-white">Latest event</p><p className="mt-1 line-clamp-3"><PlayerNames text={latestEvent}/></p></div><p className="text-[10px] leading-relaxed text-gray-500">Change the frame tactics in the centre panel at any time during Auto Play.</p></div>
