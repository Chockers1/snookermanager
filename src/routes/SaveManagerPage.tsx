...
export function SaveManagerPage() {
  const { gameState, savePending, activeSaveSlotId, listSaveSlots, saveToSlot, loadSaveSlot, deleteSaveSlot, exportCareer, importCareer } = useGame()
  ...
  return (
    <div className="space-y-4">
      ...
      {savePending && <p role="status" className="text-xs text-amber-200">Saving your latest progress. Career switching becomes available when it finishes; portable export is available now.</p>}
      <RecoverySaves />
      <div className="grid gap-4 lg:grid-cols-[1fr_0.8fr]">
        <section className="card card-body">
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button type="button" className="btn-primary" disabled={savePending} onClick={createSlot}><Save className="h-4 w-4" /> Create Copy</button>
          </div>
        ...
        <section className="card card-body">
          ...
          <div className="mt-4 grid gap-2"><button type="button" className="btn-primary justify-center" onClick={downloadSave}><Download className="h-4 w-4" /> Export Career</button><button type="button" className="btn-secondary justify-center" disabled={savePending} onClick={() => fileInputRef.current?.click()}><Upload className="h-4 w-4" /> Import Career</button><input ref={fileInputRef} className="hidden" type="file" accept="application/json,.json" onChange={(event) => void importFile(event.target.files?.[0])} /></div>
