...
const PREVIEW_RANK_BASELINES: Partial<
...
const PREVIEW_OPPONENT_ARCHETYPES = [
  "Serial Scorer",
  "Tactical Grinder",
  "Counter Puncher",
  "Tempo Disruptor",
...
function getPreviewOpponentArchetype(
  opponentName: string,
  opponentRank: number,
) {
...
function getCanonicalHistoryTotals(state: GameState) {
  ...
  return {
    matchesPlayed: completedEvents.reduce(
    ...
    prizeMoney: completedEvents.reduce(
...
function getPreviewTournamentClass(tournament: Tournament | undefined) {
  ...
  if (
    ...
    /under-|youth|junior/.test(name)
  )
    return "youth";
  if (rankingType === "Amateur" || tournament.type === "Amateur")
    return "amateur";
  ...
  if (tournament.type === "Major" && /world championship/.test(name))
    return name.includes("qualif")
      ? "worldChampionshipQualifying"
      : "worldChampionshipMain";
  if (tournament.type === "Professional Tour") return "rookieQualifier";
  return rankingType === "World Ranking" ||
...
function getPreviewRankBand(opponentRank: number, tournamentClass: string) {
  ...
  if (tournamentClass === "qTour" || tournamentClass === "qSchool")
    return "Q Tour";
  if (tournamentClass === "senior" || tournamentClass === "exhibition")
    return "Veteran Min Support";
...
function findPlayerDrawMatch(
  draw: BracketRound[],
  playerName: string,
  roundLabel?: string | null,
) {
  const rounds = roundLabel
    ? draw.filter((round) => round.label === roundLabel)
    : draw;
...
function createFallbackRankingRow(
  name: string,
  rank: number,
  nation: string,
): RankingRow {
...
function getCurrentCoach(state: GameState) {
  return (
    state.coaches.find((coach) => coach.id === state.currentCoachId) ?? null
  );
}
...
function getCurrentCue(state: GameState) {
  return state.equipment.currentCueId
    ? (cueMarketplaceCatalog.find(
        (cue) => cue.id === state.equipment.currentCueId,
...
function getCurrentCueState(state: GameState) {
  return state.equipment.currentCueId
    ? state.equipment.cueStates[state.equipment.currentCueId]
    : undefined;
}
...
function getCurrentChalk(state: GameState) {
  return state.equipment.currentChalkId
    ? (chalkCatalog.find(
        (chalk) => chalk.id === state.equipment.currentChalkId,
      ) ?? chalkCatalog[0])
    : chalkCatalog[0];
}
...
function getCurrentTip(state: GameState) {
  return state.equipment.currentTipId
    ? (tipCatalog.find((tip) => tip.id === state.equipment.currentTipId) ??
        tipCatalog[0])
    : tipCatalog[0];
}
...
function getTrendLabels(state: GameState, count: number) {
  ...
  return Array.from({ length: count }, (_, index) => ({
    ...
    cash:
      state.player.cash -
      (count - index - 1) * Math.max(0, state.finance.baseCashFlow),
  }));
}
...
function getUpcomingOpponent(state: GameState) {
  ...
  if (bracketOpponent && bracketOpponent.name !== "TBD") {
    return (
      state.rankings.find((row) => row.playerName === bracketOpponent.name) ??
      createFallbackRankingRow(
        bracketOpponent.name,
        bracketOpponent.rank,
        bracketOpponent.nation,
      )
    );
  }
  ...
  const completedOpponents =
    state.tournamentProgress.tournamentId === activeTournament?.id
      ...
      : [];
  const playerRanking =
    state.player.amateurRanking ?? state.player.worldRanking ?? 1;
...
function getResultMarginLabel(
  ...
  if (frameMargin >= 4)
    return result === "Won" ? "Convincing win" : "Heavy defeat";
...
function estimateOpponentDimension(
...
function isQfPlusRound(round: string) {
  return (
    round === "Quarter Final" || round === "Semi Final" || round === "Final"
  );
}
...
function getPressureTrait(
  ...
  if (
    ...
    return {
      label: "Weak",
      diagnosis:
        "Struggles when the match carries title or closing-frame pressure.",
    };
  }
  ...
  if (qfPlusRate >= 55 && (deciderSamples < 3 || deciderRate >= 50)) {
    return {
      label: "Strong",
      diagnosis:
        "Usually holds level once the event reaches the pressure rounds.",
    };
  }
  ...
  return {
    label: "Developing",
    diagnosis:
      "Competitive in pressure matches, but conversion is still inconsistent.",
  };
}
...
export function buildDashboardData(state: GameState) {
  ...
  const scoutedCue =
    cueMarketplaceCatalog.find(
      (cue) =>
        !state.equipment.cuesOwned.includes(cue.id) && cue.id !== currentCue.id,
    ) ??
    cueMarketplaceCatalog.find((cue) => cue.id !== currentCue.id) ??
    currentCue;
  const trend = getTrendLabels(state, 6);
  const trainingWeek: TrainingSlot[] =
    state.trainingPlan.length > 0
      ...
      : [
          ...
          {
            day: "Tue",
            morning: "Break Building",
            afternoon: "Mental Training",
            evening: "Rest",
          },
          {
            day: "Wed",
            morning: state.player.fatigue >= 60 ? "Recovery" : "Line-Up Drill",
            afternoon: "Long Pot Routine",
            evening: "Fitness",
          },
          {
            day: "Thu",
          ...
          },
          {
            day: "Fri",
            morning: "Recovery",
            afternoon: "Match Prep",
            evening: "Rest",
          },
          {
            day: "Sat",
            morning: "Match Prep",
            afternoon: "Match Simulation",
            evening: "Rest",
          },
          {
            day: "Sun",
  ...
  return {
    ...
    trainingWeek,
    financeChart,
    newsRail: state.inbox
      .slice(0, 3)
      .map((item) => ({
        id: item.id,
        tag: item.priority,
        title: item.subject,
        detail: summarizeInboxPreview(item),
      })),
  };
}
...
export function buildFinanceData(state: GameState) {
  const activeTournament = getActiveTournament(state);
  const trend = getTrendLabels(state, 6);
  const sponsorIncome = state.sponsors.reduce(
  ...
  const budgetAllocation = [
    ...
    {
      label: "Reserve",
      amount: Math.max(0, state.player.cash - totalExpenseValue),
      max: 80,
    },
  ].map((item) => ({
    ...item,
    current: clamp(Math.round((item.amount / totalBudget) * 100), 0, item.max),
  }));
  const tournamentPlanner = state.tournaments
    .filter(
      (tournament) =>
        getTournamentEntryAccess(state, tournament).allowed &&
        new Date(tournament.endDate ?? tournament.startDate).getTime() >=
      ...
      outlook:
        projectedBalance >= 12000
          ...
          : projectedBalance >= 6000
            ? "Caution"
            : "Pressure",
      trend: [0.4, 0.55, 0.65, 0.72].map((value, index) =>
        Math.round(value * 10 + month * 2 + index),
      ),
    };
  });
  const savingsValue = clamp(
    Math.round((Math.max(0, state.player.cash) / 25000) * 100),
    0,
    100,
  );
  const stabilityValue = clamp(
    100 - Math.max(0, -state.finance.cashFlow),
    20,
    100,
  );
  ...
  return {
    ...
    financialIndicators: {
      ...
      stability: {
        ...
        status:
          state.finance.cashFlow >= 0
            ? "Weekly finances are under control."
            : "Current weekly outgoings need attention.",
      },
    },
  };
}
...
export function buildMatchPreviewData(state: GameState) {
  ...
  const eventRounds =
    state.tournamentProgress.tournamentId === activeTournament?.id
      ...
      : [];
  const eventWins = eventRounds.filter(
    (round) => round.result === "Won",
  ).length;
  const eventLosses = eventRounds.filter(
    (round) => round.result === "Lost",
  ).length;
  const eventFrameDifferential = eventRounds.reduce(
    (sum, round) => sum + (round.playerFrames - round.opponentFrames),
    0,
  );
  const travelBooking = activeTournament
  ...
  const opponentProfile = state.worldPlayers.find(
    (record) => record.playerName === nextOpponent?.playerName,
  );
  const strengths = [
    {
    ...
    },
    {
      ...
      value: state.attributes.technical["Cue Ball Control"],
    },
  ...
  const weaknesses = [
    {
      label: "Long Potting",
      value: state.attributes.technical["Long Potting"],
    },
  ...
  const tacticalPlan = [
    {
      ...
      level: clamp(
        Math.round(
          (state.attributes.technical["Cue Ball Control"] +
    ...
    },
    {
      ...
      level: clamp(
        Math.round(
          (state.attributes.technical["Safety Play"] +
            state.attributes.mental.Focus) /
            2,
        ),
        35,
        90,
    ...
    },
    {
      ...
      level: clamp(
        Math.round(
          (100 - state.player.fatigue + state.attributes.physical.Stamina) / 2,
        ),
        30,
        92,
      ),
  ...
  const opponentRatings = nextOpponent
    ? getPreviewOpponentRatingSnapshot(
        nextOpponent.playerName,
        nextOpponent.ranking,
        activeRound,
        activeTournament,
        opponentProfile?.age,
      )
    : null;
  const opponentSeason = opponentProfile?.seasons.find(
    (season) => season.season === state.season,
  );
  const opponentSeasonMatches = opponentSeason?.matches ?? 0;
  const opponentWinRate =
    opponentSeasonMatches > 0
      ? (opponentSeason?.wins ?? 0) / opponentSeasonMatches
      : opponentProfile && opponentProfile.totalMatches > 0
        ? opponentProfile.wins / opponentProfile.totalMatches
        : 0.5;
  const opponentConfidence = clamp(
    Math.round(
      58 +
        (opponentWinRate - 0.5) * 54 +
        (opponentProfile?.sponsorLevel ?? 0) * 1.5,
    ),
    38,
    94,
  );
  const opponentFatigue = clamp(
    Math.round(
      opponentProfile?.fatigue ?? 18 + Math.max(0, eventRounds.length - 1) * 4,
    ),
    4,
    88,
  );
  const opponentPressure = clamp(
    Math.round(
      (100 -
        opponentConfidence +
        opponentFatigue +
        Math.max(0, 40 - (opponentRatings?.overall ?? 60) / 2)) /
        2,
    ),
    18,
    88,
  ...
  const matchAttributeComparison = [
    {
      label: "Attack",
      player: average([
        state.attributes.technical["Long Potting"],
        state.attributes.technical["Break Building"],
        state.attributes.technical["Cue Ball Control"],
      ]),
      opponent: opponentRatings
        ? average([
            opponentRatings.attributes.technical["Long Potting"],
            opponentRatings.attributes.technical["Break Building"],
            opponentRatings.attributes.technical["Cue Ball Control"],
          ])
        : null,
    },
    {
      ...
      player: average([
        state.attributes.technical["Safety Play"],
        state.attributes.mental.Focus,
        state.attributes.mental.Composure,
      ]),
      opponent: opponentRatings
        ? average([
            opponentRatings.attributes.technical["Safety Play"],
            opponentRatings.attributes.mental.Focus,
            opponentRatings.attributes.mental.Composure,
          ])
        : null,
    },
    {
      ...
      player: average([
        state.attributes.mental.Composure,
        state.attributes.mental["Big Match Nerve"],
        state.attributes.physical["Hand Steadiness"],
      ]),
      opponent: opponentRatings
        ? average([
            opponentRatings.attributes.mental.Composure,
            opponentRatings.attributes.mental["Big Match Nerve"],
            opponentRatings.attributes.physical["Hand Steadiness"],
          ])
        : null,
    },
    {
      label: "Endurance",
      player: average([
        state.attributes.physical.Stamina,
        100 - state.player.fatigue,
      ]),
      opponent: opponentRatings
        ? average([opponentRatings.attributes.physical.Stamina, 82])
        : null,
    },
  ].map((item) => ({
    ...item,
  ...
  const attributeComparison = [
    ...
    },
    {
      ...
      opponent:
        opponentRatings?.attributes.technical["Cue Ball Control"] ?? null,
    },
    {
      label: "Safety Play",
  ...
  return {
    ...
    frameDifferential: h2hMatches.reduce(
    ...
    attributeComparison,
    cueFamiliarity:
      currentCueState?.familiarity ?? currentCue?.familiarity ?? 50,
    mentalOutlook:
      state.player.fatigue >= 60
        ? "Fatigue is the main risk. Simplicity and shorter bursts of concentration matter more than forcing heavy scoring."
        : "The profile is stable enough to attack when chances appear, provided you keep the routine simple in the opening frames.",
    recentPlayerResults,
    ...
    matchInfo: {
      ...
      temperature:
        state.player.fatigue >= 60 ? "Warm / draining" : "Controlled arena",
      conditions: travelBooking
        ? "Travel booked and ready"
        : "Awaiting travel confirmation",
    },
    pressureLevel: clamp(
      Math.round((100 - state.player.confidence + state.player.fatigue) / 2),
      25,
      88,
    ),
...
export function buildMatchResultData(state: GameState) {
  ...
  const preMatchConfidence = latestMatch
    ? clamp(state.player.confidence - latestMatch.confidenceChange, 25, 99)
    : state.player.confidence;
  const preMatchFatigue = latestMatch
    ? clamp(state.player.fatigue - latestMatch.fatigueChange, 0, 100)
    : state.player.fatigue;
  const opponentTechnical = estimateOpponentDimension(
    latestMatch?.opponentStrength,
    4,
  );
  const opponentMental = estimateOpponentDimension(
    latestMatch?.opponentStrength,
    latestMatch?.round === "Final" ? 4 : 2,
  );
  const opponentPhysical = estimateOpponentDimension(
    latestMatch?.opponentStrength,
    latestMatch && latestMatch.bestOf >= 19 ? 2 : 0,
  );
  const strengthEdge =
    (latestMatch?.playerStrength ?? technicalAverage) -
    (latestMatch?.opponentStrength ?? opponentTechnical);
  const equipmentModifier = clamp(
    Math.round(
      ((cueState?.familiarity ?? cue.familiarity) - 60) / 10 +
        ((cueState?.condition ?? cue.condition) - 70) / 15,
    ),
    -4,
    6,
  );
  const confidenceModifier = clamp(
    Math.round((preMatchConfidence - 55) / 6),
    -6,
    8,
  );
  const fatigueImpact = latestMatch
    ? -clamp(
        Math.round(
          Math.max(0, preMatchFatigue - 22) /
            (latestMatch.bestOf >= 19 ? 4 : latestMatch.bestOf >= 11 ? 6 : 8),
        ),
        0,
        12,
      )
  ...
  const formatImpact = latestMatch
    ? latestMatch.bestOf >= 11
      ? clamp(
          Math.round(strengthEdge / (latestMatch.bestOf >= 25 ? 4 : 6)),
          -8,
          8,
        )
  ...
  const tacticalFit = latestMatch
    ? clamp(
        Math.round(
          ((latestMatch.potSuccess +
            latestMatch.longPotSuccess +
            latestMatch.safetySuccess) /
            3 -
            60) /
            6,
        ),
  ...
  const qfPlusMatches = recentMatches.filter((match) =>
    isQfPlusRound(match.round),
  );
  const qfPlusWins = qfPlusMatches.filter(
    (match) => match.result === "Won",
  ).length;
  const semiFinalMatches = recentMatches.filter(
    (match) => match.round === "Semi Final",
  );
  const semiFinalWins = semiFinalMatches.filter(
    (match) => match.result === "Won",
  ).length;
  const finalMatches = recentMatches.filter((match) => match.round === "Final");
  const finalWins = finalMatches.filter(
    (match) => match.result === "Won",
  ).length;
  const deciderMatches = recentMatches.filter((match) =>
    isDeciderMatch(match.bestOf, match.playerFrames, match.opponentFrames),
  );
  const deciderWins = deciderMatches.filter(
    (match) => match.result === "Won",
  ).length;
  const qfPlusRate =
    qfPlusMatches.length > 0 ? (qfPlusWins / qfPlusMatches.length) * 100 : 0;
  const finalRate =
    finalMatches.length > 0 ? (finalWins / finalMatches.length) * 100 : 0;
  const deciderRate =
    deciderMatches.length > 0 ? (deciderWins / deciderMatches.length) * 100 : 0;
  const pressureTrait = getPressureTrait(
    qfPlusRate,
    finalRate,
    deciderRate,
    finalMatches.length,
    deciderMatches.length,
  );
  const explanationSignals = [
    ...
    latestMatch && pressureImpact <= -3
      ...
      : null,
    latestMatch && fatigueImpact <= -4
      ? "Fatigue became a real drag across the later frames."
      : null,
    latestMatch && strengthEdge <= -5
      ? "The opponent started with the stronger base profile."
      : null,
    latestMatch && confidenceModifier >= 4
      ? "Confidence kept the match competitive for longer than the raw matchup suggested."
      : null,
    latestMatch && latestMatch.bestOf >= 11 && formatImpact < 0
      ? "The longer format gave the stronger baseline player more time to separate."
      : null,
    latestMatch && latestMatch.bestOf <= 5 && latestMatch.result === "Lost"
      ? "The short format left less time to correct a bad start."
      : null,
  ...
  const improvementAdvice = Array.from(
    new Set(
      [
        ...
        latestMatch && latestMatch.safetySuccess < 58
          ? "Safety Under Pressure"
          : null,
        latestMatch && latestMatch.longPotSuccess < 58 ? "Long Potting" : null,
        latestMatch &&
        latestMatch.bestOf >= 11 &&
        physicalAverage < opponentPhysical
          ? "Stamina and late-match recovery"
          : null,
        latestMatch && strengthEdge <= -5
          ? "Break Building and cue-ball control"
          : null,
      ].filter((item): item is string => Boolean(item)),
    ),
  ).slice(0, 4);
  ...
  return {
    ...
    matchSummary: latestMatch
      ? {
          ...
          expectedWinChance: Math.round(
            latestMatch.winProbability ?? latestMatch.plannedWinChance ?? 50,
          ),
          actualResult: getResultMarginLabel(
            latestMatch.playerFrames,
            latestMatch.opponentFrames,
            latestMatch.result,
          ),
        }
      : null,
    strengthBreakdown: latestMatch
      ? [
          {
            label: "Technical",
            player: technicalAverage,
            opponent: opponentTechnical,
            edge: technicalAverage - opponentTechnical,
          },
          {
            label: "Mental",
            player: mentalAverage,
            opponent: opponentMental,
            edge: mentalAverage - opponentMental,
          },
          {
            label: "Physical",
            player: physicalAverage,
            opponent: opponentPhysical,
            edge: physicalAverage - opponentPhysical,
          },
          {
            label: "Confidence",
            player: preMatchConfidence,
            opponent: clamp(
              Math.round((latestMatch.winProbability ?? 50) < 50 ? 72 : 64),
              50,
              84,
            ),
            edge:
          ...
          {
            label: "Fatigue",
            player: preMatchFatigue,
            opponent: clamp(
              Math.round(Math.max(10, preMatchFatigue - fatigueImpact - 8)),
              8,
              70,
            ),
            edge:
          ...
          {
            label: "Equipment",
            player: equipmentModifier,
            opponent: 0,
            edge: equipmentModifier,
          },
        ]
      : [],
      ...
      ? [
          {
            label:
              latestMatch.bestOf >= 11
                ? "Long format edge"
                : "Short format upset risk",
            impact:
              latestMatch.bestOf >= 11
                ? formatSignedPercent(formatImpact)
                : latestMatch.bestOf <= 5
                  ? "High"
                  : "Medium",
            detail:
              latestMatch.bestOf >= 11
                ? "Longer matches give the stronger base level more time to show."
                : "Shorter matches compress skill edge and increase volatility.",
          },
          {
            label: "Pressure impact",
            impact: formatSignedPercent(pressureImpact),
            detail: "Round stage, nerves, and confidence all feed this number.",
          },
          {
            label: "Fatigue impact",
            impact: formatSignedPercent(fatigueImpact),
            detail:
              "Higher fatigue costs more in longer matches and closing frames.",
          },
          {
            label: "Confidence boost",
            impact: formatSignedPercent(confidenceModifier),
            detail:
              "Pre-match belief shapes how much quality you can access under pressure.",
          },
          {
            label: "Equipment familiarity",
            impact: formatSignedPercent(equipmentModifier),
            detail:
              "Cue familiarity and condition still matter in close matches.",
          },
          {
            label: "Tactical fit",
            impact: formatSignedPercent(tacticalFit),
            detail:
              "Potting, safety, and long-pot execution indicate how well the plan landed.",
          },
        ]
      : [],
    resultExplanation: latestMatch
      ? {
          ...
          summary:
            explanationSignals.slice(0, 2).join(" ") ||
            "The match sat close to expectation, with no single factor overwhelming the rest of the profile.",
          detail:
            latestMatch.result === "Won"
    ...
    pressureDiagnosis: {
      ...
      deciderRecord: formatRecord(
        deciderWins,
        deciderMatches.length - deciderWins,
      ),
...
export function buildHealthCentreData(state: GameState) {
  ...
  const fatigueRisk = clamp(
    Math.round((state.player.fatigue + state.trainingCondition.strain) / 2),
    10,
    95,
  );
  const activeIssue = state.health.activeIssue;
  const bodyStatus = [
    {
      label: "Back",
      status:
        activeIssue?.bodyArea === "Back"
          ? activeIssue.severity
          : state.player.fatigue >= 65
            ? "Tight"
            : "Good",
      risk:
        activeIssue?.bodyArea === "Back" ? 85 : clamp(fatigueRisk - 10, 5, 90),
      tone:
        activeIssue?.bodyArea === "Back" || state.player.fatigue >= 65
          ? ("amber" as const)
          : ("green" as const),
    },
    {
      label: "Shoulder",
      status:
        activeIssue?.bodyArea === "Shoulder"
          ? activeIssue.severity
          : state.trainingCondition.strain >= 55
            ? "Strain Risk"
            : "Stable",
      risk:
        activeIssue?.bodyArea === "Shoulder"
          ? 88
          : clamp(state.trainingCondition.strain, 8, 88),
      tone:
        activeIssue?.bodyArea === "Shoulder" ||
        state.trainingCondition.strain >= 55
          ? ("amber" as const)
    ...
    },
    {
      label: "Wrist",
      status: "Good",
      risk: clamp(Math.round(fatigueRisk * 0.35), 5, 65),
      tone: "green" as const,
    },
    {
      label: "Sleep",
      status: state.player.morale >= 70 ? "Good" : "Interrupted",
      risk: clamp(100 - state.player.morale, 10, 85),
      tone: state.player.morale >= 70 ? ("green" as const) : ("amber" as const),
    },
    {
      label: "Mental Burnout",
      status: state.trainingCondition.burnout >= 55 ? "Elevated" : "Low Risk",
      risk: clamp(state.trainingCondition.burnout, 5, 95),
      tone:
        state.trainingCondition.burnout >= 55
          ? ("amber" as const)
          : ("green" as const),
    },
  ];
  ...
  estimatedReturnDate.setDate(
    estimatedReturnDate.getDate() + (activeIssue?.weeksRemaining ?? 0) * 7,
  );
...
export function buildSponsorshipOffersData(state: GameState) {
  const activeRevenue = state.sponsors.reduce(
    (sum, sponsor) => sum + sponsor.monthlyValue,
    0,
  );
  ...
  const slotDefinitions = [
    ...
    {
      slot: "Social Media Partner",
      unlock: "Unlock at Top 16 / 68 reputation",
    },
  ];
  const currentSlots = slotDefinitions.map((definition, index) => {
    const activeSponsor = state.sponsors.find(
      (sponsor) => sponsor.slot === definition.slot,
    );
    if (activeSponsor) {
      return {
  ...
  const brandMetrics = [
    {
      ...
      detail: "Current career profile",
    },
    {
      label: "Confidence",
      value: state.player.confidence,
      detail: "Commercial confidence signal",
    },
    {
      label: "Recent Form",
      value: clamp(
        state.player.form.filter((item) => item === "W").length * 20,
        20,
        100,
      ),
      detail: "Win trend",
    },
    {
      ...
      value: clamp(
        Math.round(
          (state.sponsors.length / Math.max(1, sponsorCapacity)) * 100,
        ),
        10,
        100,
      ),
      detail: `${state.sponsors.length}/${sponsorCapacity} active deals`,
    },
  ];
  const comparisonRows = state.sponsorOffers
    ...
    .map((offer) => ({
      ...
      valueScore: Math.max(
        ...
        Math.min(
          5,
          Math.round((offer.brandFit + offer.monthlyValue / 30) / 25),
        ),
      ),
      notes: offer.note ?? offer.bonusClause,
    }));
  const warnings = [
    state.sponsors.length >= sponsorCapacity
      ? "All unlocked sponsor slots are currently full. Improve ranking or reputation to unlock the next slot."
      : `There is room for ${sponsorCapacity - state.sponsors.length} more sponsor ${sponsorCapacity - state.sponsors.length === 1 ? "deal" : "deals"} at the current career tier.`,
    activeRevenue >= 2500
      ? "Commercial income is healthy, so fit matters more than squeezing minor extra cash."
      : "Income still needs growth; value should remain a priority.",
  ];
  ...
  return {
    ...
    advisor: {
      summary:
        activeRevenue > 0
          ? "Your commercial base is already live. Compare fit and obligation load instead of chasing headline cash only."
          : "You still need a stronger commercial floor. A stable monthly deal will smooth early-career volatility.",
      recommendation:
        state.player.reputation >= 65
          ? "Prioritise high-fit offers with manageable clauses."
          : "Take the cleanest-value deal that does not overload obligations.",
      confidence: clamp(
        Math.round((state.player.reputation + state.player.confidence) / 2),
        45,
        92,
      ),
      bulletPoints: [
...
export function buildSponsorshipContractData(
  state: GameState,
  selectedOffer: GameState["sponsorOffers"][number],
) {
  const currentRevenue = state.sponsors.reduce(
    (sum, sponsor) => sum + sponsor.monthlyValue,
    0,
  );
  const includedSlots = [
    {
      ...
      annualValue: selectedOffer.monthlyValue * 12,
      visibility: "High event exposure",
      fit: selectedOffer.brandFit,
    },
    {
      slot: "Cue Case",
      annualValue: Math.round(selectedOffer.monthlyValue * 5.5),
      visibility: "Travel / broadcast carry",
      fit: Math.max(40, selectedOffer.brandFit - 8),
    },
    {
      slot: "Digital",
      annualValue: Math.round(selectedOffer.monthlyValue * 3),
      visibility: "Social and sponsor posts",
      fit: Math.max(35, selectedOffer.brandFit - 5),
    },
  ];
  const comparisonRows = [
    {
      metric: "Monthly Payment",
      current: `£${currentRevenue.toLocaleString("en-GB")} /mo`,
      proposed: `£${selectedOffer.monthlyValue.toLocaleString("en-GB")} /mo`,
    },
    {
      metric: "Annual Value (Est.)",
      current: `£${(currentRevenue * 12).toLocaleString("en-GB")} /yr`,
      proposed: `£${(selectedOffer.monthlyValue * 12).toLocaleString("en-GB")} /yr`,
    },
    {
      metric: "Brand Fit",
    ...
    },
    {
      ...
      current: `${state.sponsors.length || 0} active deals`,
      proposed: selectedOffer.contractLength,
    },
    {
      metric: "Behaviour Clause",
      current: "Existing sponsor mix",
      proposed: selectedOffer.behaviour,
    },
    {
      metric: "Bonus Clause",
      current: "Current package average",
      proposed: selectedOffer.bonusClause,
    },
  ];
  const advisor = {
    ...
    risks: [
      ...
      selectedOffer.risk === "Risky Terms"
        ? "Terms are aggressive for the current career stage."
        : "Commercial obligations still need monitoring against training and travel load.",
    ],
    recommendation:
      selectedOffer.brandFit >= 75
        ? "Accept or make only minor asks."
        : "Negotiate first if you want stronger fit or lighter obligations.",
  };
  const terms = [
    {
      label: "Base Payment",
      details: "Monthly guaranteed payment",
      valueImpact: `+£${selectedOffer.monthlyValue.toLocaleString("en-GB")}`,
    },
    {
      label: "Bonus Clause",
      details: selectedOffer.bonusClause,
      valueImpact: "Performance upside",
    },
    {
      label: "Contract Length",
      details: selectedOffer.contractLength,
      valueImpact: "Medium-term commitment",
    },
    {
      label: "Behaviour Standard",
      details: selectedOffer.behaviour,
      valueImpact: selectedOffer.risk,
    },
  ];
  const negotiationOptions = negotiationOptionCatalog.map((option) => ({
    ...
    probability: clamp(
      option.probability +
        Math.round(
          (state.player.reputation - selectedOffer.minimumReputation) / 2,
        ),
      10,
      95,
    ),
...
export function buildMentalStateData(state: GameState) {
  ...
  const burnoutRisk = clamp(
    Math.round(
      state.trainingCondition.burnout * 0.7 + state.player.fatigue * 0.3,
    ),
    0,
    100,
  );
  const metrics = [
    {
      label: "Confidence",
      value: state.player.confidence,
      detail: state.player.confidence >= 70 ? "Strong" : "Shaky",
      tone:
        state.player.confidence >= 70
          ? ("green" as const)
          : state.player.confidence >= 55
            ? ("amber" as const)
            : ("red" as const),
    },
    {
      label: "Stress",
      value: stress,
      detail: stress >= 65 ? "Elevated" : stress >= 45 ? "Watch" : "Managed",
      tone:
        stress >= 70
          ? ("red" as const)
          : stress >= 45
    ...
    {
      label: "Motivation",
      value: state.player.morale,
      detail: state.player.morale >= 70 ? "Good" : "Fading",
      tone: state.player.morale >= 70 ? ("green" as const) : ("amber" as const),
    },
    {
      label: "Focus",
      value: state.attributes.mental.Focus,
      detail: state.attributes.mental.Focus >= 70 ? "Reliable" : "Needs Work",
      tone:
        state.attributes.mental.Focus >= 70
          ? ("green" as const)
          : ("amber" as const),
    },
    {
      ...
      value: burnoutRisk,
      detail:
        burnoutRisk >= 65 ? "High" : burnoutRisk >= 40 ? "Moderate" : "Low",
      tone:
        burnoutRisk >= 70
          ? ("red" as const)
          : burnoutRisk >= 40
    ...
    },
    {
      ...
      detail: state.attributes.mental.Composure >= 70 ? "Good" : "Below Avg",
      tone:
        state.attributes.mental.Composure >= 70
          ? ("green" as const)
          : ("amber" as const),
    },
    {
      ...
      value: clamp(
        100 -
          state.attributes.mental.Focus +
          Math.round(state.player.fatigue / 3),
        20,
        96,
      ),
      detail: state.attributes.mental.Focus >= 70 ? "Controlled" : "High Risk",
      tone:
        state.attributes.mental.Focus >= 70
          ? ("green" as const)
          : ("red" as const),
  ...
  const diagnosisSeverity = clamp(
    Math.round((metrics[1].value + metrics[6].value) / 2),
    20,
    95,
  );
  ...
  const trend = getTrendLabels(state, 6).map((point) => ({
    ...
    stress: clamp(
      Math.round((point.fatigue + (100 - point.confidence)) / 2),
      20,
      95,
    ),
  ...
  return {
    ...
    diagnosis: {
      title: recentLoss
        ? "Confidence Dip After Recent Loss"
        : "Managing Pressure Through Heavy Weeks",
      description: recentLoss
    ...
    actionPlan,
    trend,
    recoveryProgress: actionPlan.map((item, index) => ({
      label: item.title,
      value: clamp(
        70 - index * 15 + Math.round(state.player.confidence / 5),
        0,
        100,
      ),
    })),
    triggers: [
      ...
      {
        label: recentLoss
          ? `Loss vs ${recentLoss.opponentName}`
          : "Upcoming event pressure",
        timing: recentLoss?.playedOn ?? "Recently",
      },
      {
        label: `${state.sponsors.length} active commercial commitments`,
        timing: "Ongoing",
      },
    ],
    nextFocus: {
      ...
      psychologistNote:
        state.player.fatigue >= 60
          ? "Recovery should carry more weight than extra analysis this week."
          : "The current profile responds best to clarity, routine, and steady tempo.",
    },
    pressurePerformance: [
      {
    ...
    copingStrategies: [
      {
        ...
        rating: state.attributes.mental.Focus >= 70 ? 5 : 4,
      },
      {
        label: "Pre-shot Routine",
        rating: state.attributes.mental.Composure >= 70 ? 5 : 4,
      },
      {
        label: "Positive Self-Talk",
        rating: state.player.confidence >= 70 ? 4 : 3,
      },
...
export function buildTrainingPlannerData(state: GameState) {
  ...
  const drillLibrary: DrillLibraryGroup[] = [
      ...
      drills: [
        {
          name: "Line-Up Drill",
          intensity:
            state.attributes.technical["Long Potting"] >= 70
              ? "Medium"
              : "High",
        },
    ...
    },
    {
      ...
      drills: [
        ...
        {
          name: "Pressure Frames",
          intensity: state.player.confidence >= 70 ? "Medium" : "High",
        },
      ],
    },
    {
      ...
      drills: [
        ...
        { name: "Mobility", intensity: "Low" },
        {
          ...
          intensity: state.player.fatigue >= 60 ? "Low" : "Medium",
        },
      ],
    },
    {
  ...
  return {
    ...
    drillLibrary,
    enteredCompetitions,
    travelBooked: Boolean(travelBooked),
  };
}
...
export function buildCalendarData(state: GameState) {
  const events = state.tournaments.map((tournament, index) => {
    ...
    const endDate = tournament.endDate
      ? new Date(`${tournament.endDate}T00:00:00`)
      : null;
    const stageId =
      tournament.stageId ??
      (tournament.type === "Q Tour"
        ? 5
        : tournament.type === "Q School"
          ...
          : tournament.type === "Professional Tour" ||
            ...
            ? 8
            : tournament.type === "Senior" || tournament.type === "Exhibition"
              ? 14
              : tournament.type === "Amateur"
                ? 4
    ...
    return {
      ...
      month:
        tournament.month ??
        startDate.toLocaleDateString("en-GB", { month: "long" }),
      week:
        tournament.week ??
        Math.max(1, Math.min(4, Math.ceil(startDate.getDate() / 7))),
      eventClass:
      ...
      unlockRequirement:
        tournament.unlockRequirement ?? "Pathway access required.",
      progressionImpact:
        tournament.progressionImpact ?? "Builds pathway momentum.",
  ...
  return {
    ...
    getDetail(tournamentId: string) {
      ...
      return {
        alertTitle: booking
          ? "Travel booking in place"
          : "Travel still needs confirmation",
        alertText: booking
          ? "Your current travel package is already reserved for this event."
          : "Entry is only half the decision. Travel, hotel cost, and the pathway reward still need to be judged together.",
        decisionImpact: [
          {
            label: "Prize Fund",
            value: `£${(tournament.totalPrizeFund ?? tournament.prizeMoney).toLocaleString("en-GB")}`,
            tone: "green" as const,
          },
          {
            label: "Winner Prize",
            value: `£${(tournament.winnerPrize ?? Math.round(tournament.prizeMoney * 0.5)).toLocaleString("en-GB")}`,
            tone: "green" as const,
          },
          {
            label: "Total Cost",
            value: `£${(tournament.entryFee + tournament.travelCost + tournament.hotelCost).toLocaleString("en-GB")}`,
            tone: "amber" as const,
          },
        ],
        progressMeters: [
          ...
          },
          {
            ...
            value: clamp(
              Math.round(
                (state.player.confidence + (100 - state.player.fatigue)) / 2,
              ),
              0,
              100,
            ),
          ...
          {
            label: "Reputation Match",
            value: clamp(state.player.reputation, 0, 100),
            max: 100,
            detail: tournament.pathwayTier ?? "Profile fit",
            tone: "amber" as const,
          },
        ],
      };
...
export function buildTournamentHubData(state: GameState) {
  ...
  const recentResults =
    completedRounds.length > 0
      ...
      : state.matches
          .filter((match) => match.tournamentId === activeTournament?.id)
          .slice(0, 3)
          .map((match) => ({
            id: match.id,
            round: match.round,
  ...
  const upcomingMatches = state.rankings
    ...
    .map((row, index) => ({
      ...
      away:
        index === 0
          ? state.player.fullName
          : (state.rankings[(index + 2) % state.rankings.length]?.playerName ??
            "TBD"),
    }));
  const topPerformers = state.rankings
    .slice(0, 3)
    .map((row) => ({ id: row.id, name: row.playerName, score: row.points }));
  const objectives = [
    {
      label: "Win opening match",
      current: completedRounds.length > 0 ? 1 : 0,
      target: 1,
      reward: Math.max(
        12,
        Math.round((activeTournament?.rankingValue ?? 0) * 0.15),
      ),
      status: completedRounds.length > 0 ? "Complete" : "Pending",
    },
    {
      label: "Make quarter final",
      current: completedRounds.length >= 1 ? 1 : 0,
      target: 1,
      reward: Math.max(
        18,
        Math.round((activeTournament?.rankingValue ?? 0) * 0.25),
      ),
      status: completedRounds.length >= 1 ? "On track" : "Pending",
    },
    {
      label: "Hold confidence above 70",
      current: state.player.confidence >= 70 ? 1 : 0,
      target: 1,
      reward: 8,
      status: state.player.confidence >= 70 ? "Met" : "Needs work",
    },
  ];
  const notes = [
    activeTournament
    ...
    activeTournament
      ? `${activeTournament.name} carries ${activeTournament.rankingValue} ranking points.`
      : "No active tournament selected.",
    state.player.fatigue >= 60
      ? "Fatigue is the main tournament risk."
      : "Condition is stable enough to push for the next round.",
  ];

  return { recentResults, upcomingMatches, topPerformers, objectives, notes };
}

export function buildTournamentDrawData(
  state: GameState,
  tournamentId?: string | null,
) {
  const requestedTournament = tournamentId
    ? state.tournaments.find((item) => item.id === tournamentId)
    : undefined;
  const activeTournament = requestedTournament ?? getActiveTournament(state);
  const historyEntry = tournamentId
  ...
  const draw = hasMatchingProgress
    ? state.tournamentProgress.draw
    : (historyEntry?.bracket ?? []);
  const opponents = state.rankings
    .filter((row) => row.playerName !== player)
    .slice(0, 15);
  const completedRounds = hasMatchingProgress
    ? state.tournamentProgress.completedRounds
    : (historyEntry?.roundResults ?? []);
  const activeRound = hasMatchingProgress
    ? state.tournamentProgress.currentRound
    : null;
  const bracket =
    draw.length > 0
            ...
            matches: [
              {
                ...
                top: {
                  ...
                  rank:
                    ...
                    state.player.worldRanking ??
                    0,
                  nation: state.player.nationality,
                  highlighted: true,
                },
                bottom: {
                  name: opponents[0]?.playerName ?? "Opponent TBD",
                  rank: opponents[0]?.ranking ?? 0,
                  nation: opponents[0]?.nation ?? "",
                },
                placeholder: !opponents[0],
              },
            ],
          },
        ];
  ...
  const activeRoundMatches = activeRound
    ? (bracket.find((round) => round.label === activeRound)?.matches ?? [])
    : [];
  const activeMatchIndex = activeMatch
    ? activeRoundMatches.findIndex((match) => match.id === activeMatch.id)
    : -1;
  const difficultyScore =
    opponentOutlook.length > 0
      ? Math.round(
          opponentOutlook.reduce(
            (sum, opponent) =>
              sum +
  ...
  const insights = [
    ...
    {
      label: eventCompleted ? "Status" : "Current Round",
      value: eventCompleted ? "Completed" : (activeRound ?? "Awaiting draw"),
    },
    { label: "Completed Rounds", value: `${completedRounds.length}` },
    {
      label: eventCompleted ? "Your Result" : "Next Opponent",
      value: eventCompleted
        ? (historyEntry?.result ?? completedRounds.at(-1)?.round ?? "Completed")
        : (getUpcomingOpponent(state)?.playerName ?? "Awaiting result"),
    },
    {
      label: eventCompleted ? "Draw" : "Condition",
      value: eventCompleted
  ...
  return {
    ...
    progress,
    insights,
    tournamentId: activeTournament?.id ?? historyEntry?.tournamentId ?? null,
    tournamentName:
      activeTournament?.name ??
      historyEntry?.tournamentName ??
      "Tournament Draw",
    eventCompleted,
    resultLabel: historyEntry?.result ?? null,
    eventSummary: historyEntry
      ? {
          prizeMoney: historyEntry.prizeMoney,
          rankingPoints: historyEntry.rankingPoints,
          costs: historyEntry.entryFee + historyEntry.bookedTravelCost,
          net: historyEntry.prizeMoney - historyEntry.entryFee - historyEntry.bookedTravelCost,
          highestBreak: historyEntry.highestBreak,
          matchesPlayed: historyEntry.matchesPlayed,
          wins: historyEntry.wins,
          losses: historyEntry.losses,
        }
      : null,
    currentPosition: {
      currentRound: eventCompleted
        ? "Tournament complete"
        : (activeRound ?? "Awaiting draw"),
      bestResult:
        historyEntry?.result ??
        completedRounds.at(-1)?.round ??
        "No result yet",
      projectedRoute: eventCompleted
        ? "Final bracket"
        : activeMatchIndex === -1
          ? "Undrawn"
          : activeMatchIndex < activeRoundMatches.length / 2
            ? "Top half"
            : "Bottom half",
      difficultyLabel,
    },
...
export function buildSeasonReviewData(state: GameState) {
  ...
  const winRate =
    careerTotals.matchesPlayed > 0
      ? Math.round((winCount / careerTotals.matchesPlayed) * 100)
      : 0;
  ...
  const gradeScore = Math.round(
    (winRate + state.player.reputation + state.player.confidence) / 3,
  );
  const grade =
    gradeScore >= 80
      ? "A"
  ...
  return {
    ...
    grade: {
      ...
      summary:
        winRate >= 60
          ? "A strong stretch with tangible growth."
          : "Progress remains mixed but recoverable.",
    },
    objectives: [
      {
        label: "Raise ranking",
        progress: `#${state.player.amateurRanking ?? state.player.worldRanking ?? "-"}`,
        completed:
          !!state.player.amateurRanking && state.player.amateurRanking <= 4,
      },
      {
        label: "Increase sponsor base",
        progress: `${state.sponsors.length} deals`,
        completed: state.sponsors.length >= 3,
      },
      {
    ...
    panels: {
      coachGrade: {
        grade: `${grade}+`,
        detail: getCurrentCoach(state)?.name ?? "No active coach",
        note: "Coach fit now reflects the live save.",
      },
      sponsorGrade: {
        grade: state.sponsors.length >= 3 ? "A-" : "B",
        detail: `${state.sponsors.length} active sponsors`,
      ...
      verdict: {
        grade,
        title:
          winRate >= 60 ? "Season Trending Up" : "Season Needs Consolidation",
        summary: `${winCount} wins, ${lossCount} losses, and ${state.matches.length} logged matches shape the current verdict.`,
      },
      sponsorSentiment: {
        value: clamp(
          Math.round(
            (state.sponsors.length * 20 + state.player.reputation) / 2,
          ),
          20,
          100,
        ),
        detail: "Commercial sentiment",
      },
      fanSentiment: {
        value: clamp(
          Math.round((state.player.reputation + winRate) / 10),
          2,
          5,
        ),
